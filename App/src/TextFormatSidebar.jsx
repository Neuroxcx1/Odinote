    ...
    <div className="ctx-side text-format" onMouseDown={(e)=>e.preventDefault()}>
      ...
      {(!isCustomPropType || isBigTitle || isBoard) && (
        <>
          ...
          <button className={`ctx-btn ${active.bold ? 'active' : ''}`} onClick={()=> (isBigTitle || isBoard) ? onUpdate({ bold: item.bold === false }) : exec('bold')}>
            <div className="ctx-letter" style={{fontWeight: 800}}>B</div>
            <span>{lang==='es'?'Negrita':'Bold'}</span>
          </button>

          <button className={`ctx-btn ${active.italic ? 'active' : ''}`} onClick={()=> (isBigTitle || isBoard) ? onUpdate({ italic: !item.italic }) : exec('italic')}>
            <div className="ctx-letter" style={{fontStyle: 'italic', fontWeight: 700}}>I</div>
            <span>{lang==='es'?'Cursiva':'Italic'}</span>
          </button>

          <button className={`ctx-btn ${active.strike ? 'active' : ''}`} onClick={()=> (isBigTitle || isBoard) ? onUpdate({ strike: !item.strike }) : exec('strikeThrough')}>
            <div className="ctx-letter" style={{textDecoration: 'line-through', fontWeight: 700}}>S</div>
            <span>{lang==='es'?'Tachado':'Strike'}</span>
          </button>

          <button className={`ctx-btn ${active.underline ? 'active' : ''}`} onClick={()=> (isBigTitle || isBoard) ? onUpdate({ underline: !item.underline }) : exec('underline')}>
